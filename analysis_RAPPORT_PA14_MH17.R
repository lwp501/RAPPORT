# program: analysis_RAPPORT_PA14_MH17.R
...
# physical activity at age 14 on mental health at age 17
# Sections:
...
#setwd to directory
setwd("C:/Users/lwp501/research/RAPPORT/work/PA14MH17")
...
#load data
mcsData <- as.data.frame(read.csv("mcs6and7_merged_clean.csv", header=T))
...
# physical_activity_guideline_w7, physical_activity_days_w7)
# iii) pre-defined exposure (physical_activity_guideline_w6) as now define this in this code for sensitivity analysis
# iv) alternative outcome variable (kessler_SMI_w7)
...
# keep only complete cases in outcome and exposure
mcsData <- mcsData[complete.cases(mcsData[, c("kessler_w7", 
...
missing.vars.noms = c("nw_w6", "good_PE_w6",
                      ...
                      "own_bedroom_w6", "reg_watch_sport_w6", "sometimes_wake_sleep_w6")
...
missing.vars.cont = c("bmi_w6", "tot_self_esteem_w6", "tot_life_satisfaction_w6", "tot_smfq_w6",
                      "kessler_parent_w6")
...
# define outcome 
mcsData.outcome <- mcsData$kessler_w7

# define exposure

# save physical activity days variable
# ordinal variable
# Q: During last week, how often do moderate/vigorous physical activity?
# 1 = every day
# 2 = 5-6 days
# 3 = 3-4 days
# 4 = 1-2 days
# 5 = not at all
physical_activity_days_w6 <- mcsData$physical_activity_days_w6

# drop days from main dataframe 
mcsData <- dplyr::select(mcsData, !c(physical_activity_days_w6))
mcsData.imputed.noPA <- mcsData

#define all exposures

# 5 v 1/2/3/4 = none (0) v any (1)
physical_activity_any_w6<-ifelse(physical_activity_days_w6<=4, 1, 0)

# 4/5 v 1/2/3  = 2 or less (0) v 3 days+ (1)
physical_activity_3daysplus_w6<-ifelse(physical_activity_days_w6<=3, 1, 0)

# 3/4/5 v 1/2 = 4 days or less (0) v 5 days + (1) ==GUIDELINE
physical_activity_guideline_w6<-ifelse(physical_activity_days_w6<=2, 1, 0)

# 2/3/4 v 1 = 6 days or less (0) v every day (1)
physical_activity_everyday_w6<-ifelse(physical_activity_days_w6==1, 1, 0)


#add in exposure to main data frame and define separtely 
mcsData$physical_activity_guideline_w6<-physical_activity_guideline_w6

mcsData$physical_activity_guideline_w6<-physical_activity_guideline_w6

mcsData.exposure <- mcsData$physical_activity_guideline_w6

# define covariates

mcsData.covariates <- dplyr::select(mcsData, !c("kessler_w7", "physical_activity_guideline_w6"))

mcsData.no.outcome <- dplyr::select(mcsData, !c(kessler_w7))
...
selected.vars.exposure <- which(exposure.forest.varimp / mean(exposure.forest.varimp) > 0.5)
...
CreateTableOne(data=mcsData, strata = "physical_activity_guideline_w6")
CreateTableOne(data=mcsData.train, strata ="physical_activity_guideline_w6")
CreateTableOne(data=mcsData.test, strata ="physical_activity_guideline_w6")

#### 7. plot  distribution of outcome, by exposure ####

hist.outcome.all <- ggplot(mcsData, aes(kessler_w7)) + geom_bar(fill="#C55A11") + xlab("Kessler score, age 17")

hist.outcome.A0 <- ggplot(subset(mcsData,physical_activity_guideline_w6==0), aes(kessler_w7)) +
  geom_bar(fill="#A9D18E") + xlab("Kessler score, age 17, didn't meet guideline")

hist.outcome.A1 <- ggplot(subset(mcsData,physical_activity_guideline_w6==1), aes(kessler_w7)) +
  geom_bar(fill="#548235") + xlab("Kessler score, age 17, met guideline")
...
ggsave(file="outcome_distributions.png", outcomes_by_exposure) #saves g
...
par(mfrow=c(2,1))
plot(density(mcsData.test$glm.g1W[mcsData.test$physical_activity_guideline_w6==0]), 
     col = "red", main = "GLM",xlim=c(0,1))
lines(density(mcsData.test$glm.g1W[mcsData.test$physical_activity_guideline_w6==1]), 
      col = "blue", lty = 2)

plot(density(mcsData.test$ensemble.g1W[mcsData.test$physical_activity_guideline_w6==0]), 
     col = "red", main = "ensemble",xlim=c(0,1))
lines(density(mcsData.test$ensemble.g1W[mcsData.test$physical_activity_guideline_w6==1]), 
      col = "blue", lty = 2)
dev.off()
...
# Plot Relationships between CATEs and continuous variables ####

plot(loess.smooth(mcsData.covariates.test$bmi_w6, tau.hat.allvars$predictions), 
     xlab='BMI w6', ylab='CATE')

plot(loess.smooth(mcsData.covariates.test$tot_self_esteem_w6, tau.hat.allvars$predictions),
     xlab='Self esteem w6', ylab='CATE')

plot(loess.smooth(mcsData.covariates.test$tot_life_satisfaction_w6, tau.hat.allvars$predictions),
     xlab='Life satisfaction w6', ylab='CATE')

plot(loess.smooth(mcsData.covariates.test$tot_smfq_w6, tau.hat.allvars$predictions),
     xlab='SMFQ w6', ylab='CATE')

plot(loess.smooth(mcsData.covariates.test$kessler_parent_w6, tau.hat.allvars$predictions),
     xlab='Parent Kessler score w6', ylab='CATE')
...
mcsData.exposure <- physical_activity_any_w6

mcsData.exposure.train <- mcsData.exposure[train.ind]
mcsData.exposure.test <- mcsData.exposure[-train.ind]

tmle.fit.ensemble.any <- tmle(Y = mcsData.outcome.test,
                          A = mcsData.exposure.test,
                          W = mcsData.covariates.test[,selected.covs],
                          family = "gaussian",
                          V = 3,
...
mcsData.exposure <- physical_activity_3daysplus_w6

mcsData.exposure.train <- mcsData.exposure[train.ind]
mcsData.exposure.test <- mcsData.exposure[-train.ind]

tmle.fit.ensemble.3days <- tmle(Y = mcsData.outcome.test,
                              A = mcsData.exposure.test,
                              W = mcsData.covariates.test[,selected.covs],
                              family = "gaussian",
                              V = 3,
...
mcsData.exposure <- physical_activity_everyday_w6

mcsData.exposure.train <- mcsData.exposure[train.ind]
mcsData.exposure.test <- mcsData.exposure[-train.ind]

tmle.fit.ensemble.every <- tmle(Y = mcsData.outcome.test,
                                A = mcsData.exposure.test,
                                W = mcsData.covariates.test[,selected.covs],
                                family = "gaussian",
                                V = 3,
...
sens.analysis.results = data.frame(exposure = c("Any", "3+ days", "5+ days", "Every day"),
                                              ATE = c(tmle.fit.ensemble.any$estimates$ATE$psi,
                                                      tmle.fit.ensemble.3days$estimates$ATE$psi,
                                                      tmle.fit.ensemble$estimates$ATE$psi,
                                                      tmle.fit.ensemble.every$estimates$ATE$psi),
                                              lower = c(tmle.fit.ensemble.any$estimates$ATE$CI[1],
                                                        tmle.fit.ensemble.3days$estimates$ATE$CI[1],
                                                        tmle.fit.ensemble$estimates$ATE$CI[1],
                                                        tmle.fit.ensemble.every$estimates$ATE$CI[1]),
                                              upper = c(tmle.fit.ensemble.any$estimates$ATE$CI[2],
                                                        tmle.fit.ensemble.3days$estimates$ATE$CI[2],
                                                        tmle.fit.ensemble$estimates$ATE$CI[2],
                                                        tmle.fit.ensemble.every$estimates$ATE$CI[2]))
...
sens.g <- ggplot(sens.analysis.results,
                 aes(
                   ...
                   ylab("Impact on Kessler score") + xlab("Days exercise per week") + theme() +
  geom_hline(yintercept = 0, linetype='dotted', linewidth=1) +
  ggtitle("Effect of exercise at 14 on psychological distress at 17")
sens.g
