...
#setwd to directory
setwd("C:/Users/lwp501/research/RAPPORT/work/PA17MH17")
...
#physical_activity_guideline_w7, physical_activity_days_w6)
# iii) pre-defined exposure (physical_activity_guideline_w7) as now define this in this code for sensitivity analysis
...
missing.vars.noms = c("nw_w6", "good_PE_w6",
                      ...
                      "own_bedroom_w6", "own_computer_w6", "reg_watch_sport_w6",
...
# 4 = 1-2 days
# 5 = not at all
physical_activity_days_w7 <- mcsData$physical_activity_days_w7

# drop days from main dataframe 
mcsData <- dplyr::select(mcsData, !c(physical_activity_days_w7))
mcsData.imputed.noPA <- mcsData

#define all exposures

# 5 v 1/2/3/4 = none (0) v any (1)
physical_activity_any_w7<-ifelse(physical_activity_days_w7<=4, 1, 0)

# 4/5 v 1/2/3  = 2 or less (0) v 3 days+ (1)
physical_activity_3daysplus_w7<-ifelse(physical_activity_days_w7<=3, 1, 0)

# 3/4/5 v 1/2 = 4 days or less (0) v 5 days + (1) ==GUIDELINE
physical_activity_guideline_w7<-ifelse(physical_activity_days_w7<=2, 1, 0)

# 2/3/4 v 1 = 6 days or less (0) v every day (1)
physical_activity_everyday_w7<-ifelse(physical_activity_days_w7==1, 1, 0)


#add in exposure to main data frame and define separtely 
mcsData$physical_activity_guideline_w7<-physical_activity_guideline_w7

mcsData$physical_activity_guideline_w7<-physical_activity_guideline_w7

mcsData.exposure <- mcsData$physical_activity_guideline_w7

# define covariates

mcsData.covariates <- dplyr::select(mcsData, !c("kessler_w7", "physical_activity_guideline_w7"))
...
CreateTableOne(data=mcsData, strata = "physical_activity_guideline_w7")
CreateTableOne(data=mcsData.train, strata ="physical_activity_guideline_w7")
CreateTableOne(data=mcsData.test, strata ="physical_activity_guideline_w7")

#### 7. plot  distribution of outcome, by exposure ####

hist.outcome.all <- ggplot(mcsData, aes(kessler_w7)) + geom_bar(fill="#C55A11") + xlab("Kessler score, age 17")

hist.outcome.A0 <- ggplot(subset(mcsData,physical_activity_guideline_w7==0), aes(kessler_w7)) +
  geom_bar(fill="#A9D18E") + xlab("Kessler score, age 17, didn't meet guideline")

hist.outcome.A1 <- ggplot(subset(mcsData,physical_activity_guideline_w7==1), aes(kessler_w7)) +
...
par(mfrow=c(2,1))
plot(density(mcsData.test$glm.g1W[mcsData.test$physical_activity_guideline_w7==0]), 
     col = "red", main = "GLM",xlim=c(0,1))
lines(density(mcsData.test$glm.g1W[mcsData.test$physical_activity_guideline_w7==1]), 
      col = "blue", lty = 2)

plot(density(mcsData.test$ensemble.g1W[mcsData.test$physical_activity_guideline_w7==0]), 
     col = "red", main = "ensemble",xlim=c(0,1))
lines(density(mcsData.test$ensemble.g1W[mcsData.test$physical_activity_guideline_w7==1]), 
      col = "blue", lty = 2)
dev.off()
...
mcsData.exposure <- physical_activity_any_w7
...
mcsData.exposure <- physical_activity_3daysplus_w7
...
mcsData.exposure <- physical_activity_everyday_w7
...
sens.g <- ggplot(sens.analysis.results,
  ...
  ggtitle("Effect of exercise at 17 on psychological distress at 17")
sens.g
